...
const copy = {
  sk: {
    ...
    subtitle: 'Príďte nás navštíviť, radi vás privítame.',
    quote: 'Lepšie raz vidieť ako sto krát počuť.',
    where: 'Kde nás nájdete',
    address: 'Adresa',
    email: 'Email',
    phone: 'Telefón',
    ...
    follow: 'Sledujte nás',
    mapTitle: 'Wellness Heaven - Tomášikova, Bratislava',
  },
  en: {
    title: 'Contact Information',
    subtitle: 'Visit us, we will be happy to welcome you.',
    quote: 'Seeing once is better than hearing a hundred times.',
...
export default async function KontaktLocalizedPage({ params }: PageProps) {
  const { locale } = await params;
  if (!isLocale(locale)) {
    notFound();
  }
    ...
    <div className="pt-20 sm:pt-24 lg:pt-32 pb-16 sm:pb-20 md:pb-24 lg:pb-32 bg-[#faf9f7] min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16">
          ...
          <p className="text-lg sm:text-xl text-[#6b6b6b] mb-2">{t.subtitle}</p>
          <p className="text-base sm:text-lg text-[#6bb8ff] italic font-display">{t.quote}</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 sm:p-10 md:p-12 border border-[#e8e6e3] text-center space-y-3">
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="space-y-2">
                <p className="text-[#2c2c2c] font-semibold text-sm sm:text-base">{t.address}</p>
              ...
              <div className="space-y-2">
                <p className="text-[#2c2c2c] font-semibold text-sm sm:text-base">{t.email}</p>
                <a
                  href="mailto:wellnessheavensk@gmail.com"
                  className="text-[#6bb8ff] hover:text-[#4d9be0] transition-colors font-medium text-sm sm:text-base break-all"
                >
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 sm:p-10 md:p-12 border border-[#e8e6e3] text-center space-y-4">
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="pb-4 sm:pb-6 border-b border-[#e8e6e3]">
                <p className="text-[#6b6b6b] font-medium mb-2 text-sm sm:text-base">{t.weekdays}</p>
                <p className="text-[#2c2c2c] text-xl sm:text-2xl font-display font-bold">11:00 – 22:00</p>
              </div>
              <div>
                <p className="text-[#6b6b6b] font-medium mb-2 text-sm sm:text-base">{t.weekends}</p>
